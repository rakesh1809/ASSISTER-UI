pipeline {
    ...
    environment {
        ...
        NEXUS_URL = 'http://hios-devops-nexus.hios-mgmt.mp.cmscloud.local:8081'
    ...
    stages {
        ...
        stage('Checkout for Module') {
            steps {
                echo 'Checkout SCM....'
                git branch: "${BRANCH}", credentialsId: "e4a25c3e-1eda-4b9b-b1ea-36748dba3d42", url: "https://github.cms.gov/HIOS/${BRANCH_NAME}.git"
        ...
        stage('Set artifact') {
            steps {
                script {
                    env.MODULE = sh(script: "ls ${WORKSPACE}/${JOB_NAME}-${BUILD_ID}/build-artifacts", returnStdout: true)
                }
            }
        }
        stage('Deploy to DEV') {
